...
public class ATProtoTools {
    ...
    public func isValidReplyReference(_ reference: AppBskyLexicon.Feed.PostRecord.ReplyReference, session: UserSession? = nil) async -> Bool {
        ...
        do {
            ...
            _ = try await ATProtoKit(pdsURL: session?.pdsURL ?? ATProtoKit.publicAPI, canUseBlueskyRecords: false).getRepositoryRecord(
                from: repository,
    ...
    public func createReplyReference(
        ...
        do {
            ...
            let record = try await ATProtoKit(pdsURL: session.pdsURL ?? ATProtoKit.publicAPI, canUseBlueskyRecords: false).getRepositoryRecord(
                from: repository,
                collection: collection,
                recordKey: recordKey
            )
